/*
  LCD5110_SSVS.cpp - Arduino/chipKit library support for Nokia 5110 compatible LCDs
...
void LCD5110::print(char *st, int x, int y, int length, char filler)
{
	unsigned char ch;
	int stl, row, xp;
	...
	if (!_sleep)
	{
			...
			if (length > stt)
			{
			for (int yy=0; yy<(length-stt); yy++) st=filler+st;
...
void LCD5110::print(String st, int x, int y, int length, char filler)
{
	byte stl = st.length();
	if (length > stl)
			{
			for (byte yy=0; yy < (length - stl); yy++) st=filler+st;
			}
